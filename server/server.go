...
服务端的职责：
1. 监听来自本地代理客户端的请求
2. 解密本地代理客户端请求的数据，解析 SOCKS5 协议，链接用户浏览器真正想要链接的远程服务器
3. 转发远程服务器返回的数据，加密后，转发到本地代理客户端
*/

func NewRsServer(password string, listenAAddr string) (*RsServer, error)  {
	pw, err := tools.ParsePassword(password)
	if err != nil {
		return nil, err
	}
	structListenAddr, err := net.ResolveTCPAddr("tcp", listenAAddr)
	if err != nil {
		return nil, err
	}
	return &RsServer{
